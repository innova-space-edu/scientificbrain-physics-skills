## Description
Orchestrate the complete validated plasma workflow: regime assessment → FLASH setup/run/sweep → HDF5/yt → PhysicsNeMo dataset → 2D/3D surrogate training/inference → comparison against held-out FLASH and physical constraints.
...
## Use case
Use when an agent needs to orchestrate the complete validated plasma workflow: regime assessment → FLASH setup/run/sweep → HDF5/yt → PhysicsNeMo dataset → 2D/3D surrogate training/inference → comparison against held-out FLASH and physical constraints.
